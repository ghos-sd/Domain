...
MIN_INTERVAL = float(os.getenv("MIN_INTERVAL", "1.0"))  # ثانية بين طلبات Playwright
sem = asyncio.Semaphore(MAX_CONCURRENCY)
_last_call_ts = 0.0

# Telegram
TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN")        # أضِفها في Railway → Variables
...
class CheckResult(BaseModel):
    ...
    price: Optional[str] = None # example "$8.88/yr"
...
async def root():
    return {
        ...
        "endpoints": ["/health", "/check?domain=example.com", "/webhook (POST)", "/telegram-webhook (POST)"]
    }
...
async def _handle_telegram_update(data: dict) -> None:
...
async def telegram_webhook(req: Request, x_telegram_bot_api_secret_token: str | None = Header(default=None)):
    ...
    data = await req.json()
    await _handle_telegram_update(data)
    return {"ok": True}

# alias عشان لو سجلت المسار /telegram-webhook
@app.post("/telegram-webhook")
async def telegram_webhook_alias(req: Request, x_telegram_bot_api_secret_token: str | None = Header(default=None)):
    return await telegram_webhook(req, x_telegram_bot_api_secret_token)            page = await context.new_page()
            try:
                await page.goto(url, wait_until="domcontentloaded", timeout=15000)
                await page.wait_for_load_state("networkidle", timeout=15000)
                body_text = await page.inner_text("body")
